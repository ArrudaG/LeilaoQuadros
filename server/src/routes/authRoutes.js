...
authRoutes.post('/register', async (req, res) => {
  ...
  const { cpf, email, phone, birthDate, firstName, lastName, password, biometricEnabled } = parsed.data;
  ...
  try {
    ...
    const created = await pool.query(
      `INSERT INTO leilao_users (cpf, user_role, email, birth_date, phone, first_name, last_name, password_hash, biometric_enabled)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
       RETURNING id, cpf, user_role, email, birth_date, phone, first_name, last_name, profile_image_url, biometric_enabled`,
      [cpf, role, email, birthDate, phone, firstName, lastName, passwordHash, biometricEnabled],
    );
...
authRoutes.post('/login', async (req, res) => {
  ...
  try {
    const found = await pool.query(
      `SELECT id, cpf, user_role, email, birth_date, phone, first_name, last_name, profile_image_url, password_hash, biometric_enabled
       FROM leilao_users
       WHERE cpf = $1`,
      [cpf],
    );
...
authRoutes.get('/me', authMiddleware, async (req, res) => {
  try {
    const found = await pool.query(
      `SELECT id, cpf, user_role, email, birth_date, phone, first_name, last_name, profile_image_url, biometric_enabled
       FROM leilao_users
       WHERE id = $1`,
      [req.user.sub],
    );
